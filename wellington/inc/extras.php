 ...
 * @return void
 */
function wellington_retina_logo() {
	// Return early if there is no logo image or option for retina logo is disabled.
	if ( ! has_custom_logo() or false === wellington_get_option( 'retina_logo' ) ) {
		return;
	}
	...
	// Add Custom CSS.
	wp_add_inline_style( 'wellington-stylesheet', $css );
}
add_filter( 'wp_enqueue_scripts', 'wellington_retina_logo', 11 );
