 ...
 */
function wellington_amp_menu_toggle() {
 ...
 */
function wellington_amp_menu_is_toggled() {
	if ( wellington_is_amp() ) {
 ...
 */
function wellington_amp_menu_dropdown_toggles( $item_output, $item, $depth, $args ) {
	...
	*/
	...
	// Add SVG icon.
	$dropdown_button .= $expanded ? wellington_get_svg( 'collapse' ) : wellington_get_svg( 'expand' );
	...
	$dropdown_button .= sprintf(
		...
		esc_attr( sprintf( "$expanded_state_id ? %s : %s", wp_json_encode( esc_html__( 'Collapse child menu', 'wellington' ) ), wp_json_encode( esc_html__( 'Expand child menu', 'wellington' ) ) ) ),
		esc_html( $expanded ? esc_html__( 'Collapse child menu', 'wellington' ) : esc_html__( 'Expand child menu', 'wellington' ) )
	);
...
add_filter( 'walker_nav_menu_start_el', 'wellington_amp_menu_dropdown_toggles', 10, 4 );
