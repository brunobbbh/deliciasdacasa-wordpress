/* global wellingtonScreenReaderText */
/**
 * Theme Navigation
 *
 * @package Wellington
 */
 ...
 (function() {
	...
	function createDropdownToggle() {
		...
		var icon = new DOMParser().parseFromString( wellingtonScreenReaderText.icon, 'text/html' ).body.firstElementChild;
		dropdownToggle.appendChild( icon);

		// Add screenreader text.
		var screenReaderText = document.createElement( 'span' );
		screenReaderText.classList.add( 'screen-reader-text' );
		screenReaderText.textContent = wellingtonScreenReaderText.expand;
	...
	function initNavigation( containerClass, naviClass ) {
		...
		(function() {
			...
			navigation.querySelectorAll( '.current-menu-ancestor > button' ).forEach( function( activeToggle ) {
				...
				activeToggle.querySelector( '.screen-reader-text' ).textContent = wellingtonScreenReaderText.collapse;
			});
			...
			navigation.querySelectorAll( '.dropdown-toggle' ).forEach( function( dropdownItem ) {
				dropdownItem.addEventListener( 'click', function() {
					...
					dropdownItem.querySelector( '.screen-reader-text' ).textContent = dropdownItem.classList.contains( 'toggled-on' ) ? wellingtonScreenReaderText.collapse : wellingtonScreenReaderText.expand;
